...
if (componentName === 'component') {
  console.log(
    chalk
      .hex('#cb3837')
      .bold(
        'Error: You did not enter the component name, You should run:'
      )
  )
  console.log(
    chalk.hex('#13c2c2').bold('yarn create-component [component-name]')
  )
  return
}
...
inquirer
  ...
  .then((value) => {
    if (value.isSure) {
      ...
      console.log(
        chalk.hex('#13c2c2').bold('You has create component success!')
      )
    } else {
...
function importIndex() {
  const str = `import U${UpperComponentName} from './${componentName}/index.js'\n`

  let result = fs.readFileSync('./src/index.ts', 'utf8')

  const id1 = result.match(/\/\/ImportFlag/)
  ...
  const id2 = result.match(/\/\/ArrayFlag/)
  res = result.split('')
  res.splice(id2.index, 0, `U${UpperComponentName},\n\t`)

  result = res.join('')

  const id3 = result.match(/\/\/ExportFlag/)
  res = result.split('')
...
function importStyle() {
  const str = `@import './${componentName}.styl'\n`
...
function importDocsConfig() {
  const str = `{ text: '${UpperComponentName}', link: '/components/${componentName}/'},\n\t\t\t\t\t\t`
  const str2 = `{ text: '${UpperComponentName}', link: '/en/components/${componentName}/'},\n\t\t\t\t\t\t`

  let result = fs.readFileSync('./docs/.vitepress/config.js', 'utf8')
  const id = result.match(/\/\/Flag/)

  let res = result.split('')
  res.splice(id.index, 0, str)

  result = res.join('')
  const id2 = result.match(/\/\/Flag2/)
